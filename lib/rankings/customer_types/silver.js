        ...
        return await fetch(`https://www.random.org/integers/?num=1&min=0&max=${getAppsDetailsLength() - 1}&col=1&base=10&format=plain&rnd=new`)
            .then(res => {
                if(res.status.toString()[0] !== 2){
                    throw new Error("probably got banned from random.org address,  goto catch fallback")
                }
                return res.text()
            })
            .then(res => {
            ...
            .catch(err =>{
                ...
                return Object.create(Bronze.prototype).getUniqueRandomNumbers().values().next().value;
            })
